...
fn main() {
    ...
    let dir = Path::new(&home::home_dir().unwrap()).join(".maildisk");
    let config_content = fs::read_to_string(&Path::new(&dir).join("config.json")).unwrap();
    let conf = json::parse(&config_content).unwrap();
